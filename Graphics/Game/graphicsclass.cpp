...
#include "Engine/Rendering/UI/bitmapclass.h"
#include "Engine/Scene/cameraclass.h"
#include "Engine/Scene/CollisionHelpers.h"
#include "Engine/Rendering/Device/d3dclass.h"
#include "Engine/Rendering/Shaders/FireShaderClass.h"
#include "Engine/Rendering/Geometry/foliageclass.h"
#include "Engine/Rendering/Shaders/foliageshaderclass.h"
#include "Engine/Input/inputclass.h"
#include "Engine/Scene/LightClass.h"
#include "Engine/Rendering/Geometry/modelclass.h"
#include "Engine/Rendering/Geometry/ModelClassForNoiseFilter.h"
#include "Engine/Rendering/Geometry/SkyDomeClass.h"
#include "Engine/Rendering/Shaders/SkyDomeShaderClass.h"
#include "Engine/Audio/SoundClass.h"
#include "Engine/Rendering/Geometry/TerrainClass.h"
#include "Engine/Rendering/UI/textclass.h"
#include "Engine/Rendering/Shaders/textureshaderclass.h"
#include "Engine/Input/FirstPersonCameraController.h"
#include "Engine/Rendering/Pipelines/SoftShadowPipeline.h"
...
namespace
{
	Engine::Result<void> InitializationFailure(
		std::string message,
		Engine::SourceLocation location)
	{
		return Engine::Result<void>::Failure(Engine::Error(
			Engine::ErrorCode::InitializationFailed,
			"PortfolioGame",
			std::move(message),
			location));
	}

	Engine::Result<Engine::UpdateResult> UpdateFailure(
		std::string message,
		Engine::SourceLocation location)
	{
		return Engine::Result<Engine::UpdateResult>::Failure(Engine::Error(
			Engine::ErrorCode::UpdateFailed,
			"PortfolioGame",
			std::move(message),
			location));
	}

	Engine::Result<void> RenderFailure(
		std::string message,
		Engine::SourceLocation location)
	{
		return Engine::Result<void>::Failure(Engine::Error(
			Engine::ErrorCode::RenderFailed,
			"PortfolioGame",
			std::move(message),
			location));
	}
...
#define PORTFOLIO_INITIALIZATION_FAILURE(message) \
	InitializationFailure((message), ENGINE_SOURCE_LOCATION)
#define PORTFOLIO_UPDATE_FAILURE(message) \
	UpdateFailure((message), ENGINE_SOURCE_LOCATION)
#define PORTFOLIO_RENDER_FAILURE(message) \
	RenderFailure((message), ENGINE_SOURCE_LOCATION)
...
Engine::Result<void> GraphicsClass::Initialize(const Engine::NativeWindow& window)
{
	return InitializeResources(window.width, window.height, window.handle, window.instance);
}

Engine::Result<void> GraphicsClass::InitializeResources(
	int screenWidth,
	int screenHeight,
	HWND hwnd,
	HINSTANCE hInstance)
{
	...
	if(!m_d3d)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the Direct3D device wrapper.");
	}
	...
	if(!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize Direct3D.");
	}
	...
	if(!m_camera)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the camera.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the camera.");
	}
	...
	if (!m_input)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the input service.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the input service.");
	}
	...
	for (const auto& definition : m_sceneDefinition.gameObjects)
	{
		...
		if (!result)
		{
			return PORTFOLIO_INITIALIZATION_FAILURE(
				"Could not initialize scene model at index " +
				std::to_string(m_gameObjects.size()) + '.');
		}
	...
	if (!m_fireModel)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the fire model.");
	}
	...
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the fire model.");
	}
	m_objectCount++;
	...
	if (!m_fireShader)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the fire shader.");
	}

	// 화재 쉐이더 객체를 초기화합니다.
	if (!m_fireShader->Initialize(m_d3d->GetDevice(), hwnd))
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the fire shader.");
	}

	m_textureShader = Engine::MakeScopedResource<TextureShaderClass>();
	if (!m_textureShader)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the texture shader.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the texture shader.");
	}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (!m_crosshairs[i])
		{
			return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate a crosshair bitmap.");
		}
		...
		if (!result)
		{
			return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize a crosshair bitmap.");
		}
	...
	if (!m_mouseCursor)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the mouse cursor bitmap.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the mouse cursor bitmap.");
	}
	...
	if (!m_text)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the text service.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the text service.");
	}
	...
	if (!m_terrain)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the terrain.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the terrain.");
	}
	...
	if (!m_light)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the light.");
	}
	...
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the soft-shadow pipeline.");
	}
	...
	if (!m_skyDome)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the sky dome.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the sky dome.");
	}
	else{
		m_objectCount++;
	...
	if (!m_skyDomeShader)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the sky-dome shader.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the sky-dome shader.");
	}
	...
	if (!m_foliage)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the foliage system.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the foliage system.");
	}
	...
	if (!m_foliageShader)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the foliage shader.");
	}
	...
	if (!result)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize the foliage shader.");
	}
	...
	if (!m_sound)
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not allocate the audio service.");
	}
	...
	{
		return PORTFOLIO_INITIALIZATION_FAILURE("Could not initialize DirectSound.");
	}
...
Engine::Result<Engine::UpdateResult> GraphicsClass::FixedUpdate(
	const Engine::FixedFrameContext& frame)
{
	return UpdateScene(frame);
}

Engine::Result<void> GraphicsClass::Render(
	const Engine::RenderFrameContext&)
{
	if (!RenderScene())
	{
		return PORTFOLIO_RENDER_FAILURE("A scene render pass failed.");
	}
	if (!m_d3d->EndScene())
	{
		return PORTFOLIO_RENDER_FAILURE("Presenting the swap chain failed.");
	}
	return Engine::Result<void>::Success();
}

Engine::Result<Engine::UpdateResult> GraphicsClass::UpdateScene(
	const Engine::FixedFrameContext& frame)
{
	const float frameTime = frame.deltaMilliseconds;
	const Engine::Result<Engine::UpdateResult> inputResult = HandleInput(frameTime);
	if (!inputResult)
	{
		return Engine::Result<Engine::UpdateResult>::Failure(inputResult.GetError());
	}
	if (inputResult.Value() != Engine::UpdateResult::Continue)
	{
		return Engine::Result<Engine::UpdateResult>::Success(inputResult.Value());
	}
	...
	case Stage::Title:
		...
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the title text buffer.");
		}
		break;
	...
	case Stage::Hunt:
		...
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the FPS text buffer.");
		}

		if (!m_text->SetCpu(frame.cpuPercentage, m_d3d->GetDeviceContext()))
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the CPU text buffer.");
		}

		if (!m_text->SetPolygons(m_polygonCount, m_d3d->GetDeviceContext()))
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the polygon-count text buffer.");
		}

		if (!m_text->SetObject(m_objectCount, m_d3d->GetDeviceContext()))
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the object-count text buffer.");
		}

		if (!m_text->SetSizeOfScreen(m_d3d->GetDeviceContext()))
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the screen-size text buffer.");
		}
		break;
	...
	m_fireAnimationTime += frame.fixedDeltaSeconds;
	...
	if (m_stage == Stage::Hunt)
	{
		...
		if (!m_foliage->Frame(
			...
			m_terrain.get()))
		{
			return PORTFOLIO_UPDATE_FAILURE("Could not update the foliage simulation.");
		}
	}
	...
	return Engine::Result<Engine::UpdateResult>::Success(
		Engine::UpdateResult::Continue);
}

Engine::Result<Engine::UpdateResult> GraphicsClass::HandleInput(float frameTime)
{
	if (!m_input->Frame(frameTime))
	{
		return PORTFOLIO_UPDATE_FAILURE("Could not acquire the current input state.");
	}
	...
	{
		return Engine::Result<Engine::UpdateResult>::Success(
			Engine::UpdateResult::ExitRequested);
	}
