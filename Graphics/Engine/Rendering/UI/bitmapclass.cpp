...
{
	Shutdown();
	if (device == nullptr || textureFilename == nullptr)
	{
		return false;
	}
...
void BitmapClass::SetPosition(float x, float y, float z)
{
	m_position.x = x;
	m_position.y = y;
	m_position.z = z;
...
void BitmapClass::SetScale(float x, float y, float z)
{
	m_scale.x = x;
	m_scale.y = y;
	m_scale.z = z;
...
void BitmapClass::GetPosition(float& x, float& y, float& z)
{
	x = m_position.x;
	y = m_position.y;
	z = m_position.z;
...
void BitmapClass::GetScale(float& x, float& y, float& z)
{
	x = m_scale.x;
	y = m_scale.y;
	z = m_scale.z;
...
bool BitmapClass::InitializeBuffers(ID3D11Device* device)
{
	...
	HRESULT result = device->CreateBuffer(&vertexBufferDesc, &vertexData, vertexBuffer.GetAddressOf());
	if(FAILED(result))
	{
		return false;
	}
...
bool BitmapClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	memcpy(verticesPtr, vertices.data(), (sizeof(VertexType) * m_vertexCount));
