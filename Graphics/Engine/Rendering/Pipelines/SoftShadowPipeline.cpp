...
#include "Engine/Rendering/Shaders/HorizontalBlurShaderClass.h"
#include "Engine/Scene/LightClass.h"
#include "Engine/Rendering/Geometry/OrthoWindowClass.h"
#include "Engine/Rendering/Shaders/ShadowShaderClass.h"
#include "Engine/Rendering/Shaders/SoftShadowShaderClass.h"
#include "Engine/Rendering/Geometry/TerrainClass.h"
#include "Engine/Rendering/Shaders/textureshaderclass.h"
#include "Engine/Rendering/Shaders/VerticalBlurShaderClass.h"
...
namespace Engine::Rendering
{
		...
		const SoftShadowPipelineConfig& config)
	{
		...
		const auto createRenderTexture = [device](
			int width,
			int height,
			float screenDepth,
			float screenNear) -> RenderTexturePtr
		{
			return MakeTexture<RenderTargetTexture>(
				device,
				RenderTextureDescriptor{ width, height, screenDepth, screenNear });
		};

		m_shadowTexture = createRenderTexture(
		...
		m_blackWhiteShadowTexture = createRenderTexture(
			config.shadowMapWidth,
			config.shadowMapHeight,
			config.screenDepth,
			config.screenNear);
		if (!m_blackWhiteShadowTexture)
		{
		...
		m_downsampleTexture = createRenderTexture(downSampleWidth, downSampleHeight, 100.0f, 1.0f);
		...
		m_upsampleTexture = createRenderTexture(
			config.shadowMapWidth,
			config.shadowMapHeight,
			config.screenDepth,
			0.1f);
